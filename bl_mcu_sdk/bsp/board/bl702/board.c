...
static void board_pin_mux_init(void)
{
    ...
    for (int i = 0; i < sizeof(af_pin_table) / sizeof(af_pin_table[0]); i++) {
        ...
        /*if using gpio9-gpio12 input mode or wakeup,should set reg_aon_pad_ie_smt corresponding bit = 1*/
        if ((af_pin_table[i].pin > GPIO_PIN_8) && (af_pin_table[i].pin < GPIO_PIN_13) && (af_pin_table[i].func >= GPIO_FUN_GPIO_INPUT_UP) && (af_pin_table[i].func <= GPIO_FUN_WAKEUP)) {
